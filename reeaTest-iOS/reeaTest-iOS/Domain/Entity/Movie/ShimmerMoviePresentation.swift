//
//  ShimmerMoviewPresentation.swift
//  ReeaTest-iOS
//
...
struct ShimmerMoviePresentation: Codable, AbstractDomainEntity {
    let id: Int?
    ...
    var asCellViewModel: AbstractCellViewModel {
        return ShimmerMoviewCellViewModel(id: id, thumbnail: "\(AppConfig.shared.getServerConfig().getMediaBaseUrl())/\(posterPath ?? "" )", title: title, overview: overview)
    }
    ...
    var asCellConfigurator: CellConfigurator {
        return ShimmerItemCellConfig(item: asCellViewModel)
    }
}
